...
while True:
    ...
    prompt = usrHistory + "\n" +predict.choices[0].text.strip()+ "\nthe Little white fox is talking to "+ playerName+ "\n" + "Little white fox: ",
    temperature = 1.2,
    frequency_penalty=0,
    presence_penalty=0.6,
    top_p=1,
    ...
    if(usrInput == playerName + ": stop"):
        ...
        print("Running cost: $"+ str((davinciTokens*0.02 + curieTokens*0.002)/1000))

        print("Average cost: $"+ str((davinciTokens*0.02 + curieTokens*0.002)*(1/totalConversation)/1000) )
        break
